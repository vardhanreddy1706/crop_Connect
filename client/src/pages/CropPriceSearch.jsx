...
// 						<p>
// 							<strong>Showing:</strong> {startIndex + 1}-{endIndex} of{" "}
// 							{filteredData.length} results (total dataset: {priceData.length})
// 						</p>
...
// 																{arrow} {Math.abs(trendNum).toFixed(2)}%
// 															</span>
// 														);
// 													})()}
// 												</td>
...
const CropPriceSearch = () => {
	...
	const fetchPriceData = async () => {
		...
		try {
			...
			if (data.records) {
				...
				setUniqueValues({ states, commodities, markets, districts: [] });
			} else {
				setError("No price data found");
			}
		} catch (err) {
	...
	useEffect(() => {
		if (searchFilters.state) {
			...
			setUniqueValues((prev) => ({ ...prev, districts }));
	...
	// Reset filters
	const handleResetFilters = () => {
		setSearchFilters({ state: "", district: "", commodity: "", market: "" });
		setFilteredData(priceData);
		setCurrentPage(1);
	};
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 p-4 md:p-8">
			<div className="max-w-7xl mx-auto">
				...
				{/* ===== Responsive Table with INTERNAL horizontal scroll ===== */}
				{!loading && !error && filteredData.length > 0 && (
					<div className="bg-white rounded-xl shadow-lg border border-green-100">
						{/* Scrollbar lives INSIDE this card on small screens */}
						<div className="relative max-w-full overflow-x-auto md:overflow-x-visible">
							<table className="w-full min-w-[1100px] table-auto">
								<thead className="sticky top-0 z-10">
									<tr className="border-b border-gray-200 bg-gradient-to-r from-green-600 to-emerald-600">
										{[
											"State",
											"District",
											"Market",
									...
									{currentRows.map((item, idx) => {
										...
										const color =
											trendNum > 0
												...
												? "text-red-600"
												: "text-gray-600";
										const arrow = trendNum > 0 ? "▲" : trendNum < 0 ? "▼" : "→";
						...
						{/* Pagination (outside the scroll area) */}
						{filteredData.length > 0 && (
							<div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-gray-200 bg-gray-50">
								<div className="text-sm text-gray-600">
									Page {currentPage} of {totalPages}
