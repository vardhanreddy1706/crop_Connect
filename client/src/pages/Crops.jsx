...
const buildBadges = (crop) => {
	...
	if (qtl < 1 && qtl > 0)
		badges.push({ text: "Insufficient", cls: "bg-orange-500" });

	if (status === "sold" || qtl <= 0)
		badges.push({ text: "Sold out", cls: "bg-red-700" });
	if (status === "sold")
		badges.push({ text: "Completed", cls: "bg-purple-600" });
	if (status === "pending")
		badges.push({ text: "Not available", cls: "bg-yellow-600" });
	if (status === "unavailable" || status === "not available")
		badges.push({ text: "Unavailable", cls: "bg-gray-600" });
	...
	return (
		<div className="min-h-screen bg-fixed bg-gradient-to-br from-emerald-100 via-green-100 to-sky-100">
			<div className="max-w-6xl mx-auto px-4 py-8">
				...
				{safeCrops.length === 0 ? (
					<div className="flex flex-col items-center justify-center py-16 px-4 text-center bg-white/80 backdrop-blur rounded-lg shadow-md">
						...
						<p className="text-gray-600 mb-6 max-w-md">
							There are currently no crops listed. Check back later or browse other sections.
						</p>
						<button
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						{safeCrops.map((crop) => {
							...
							const coverImg = Array.isArray(crop?.images)
								? typeof crop.images[0] === "string"
									? crop.images[0]
									: crop.images[0]?.url
								: crop?.image || placeholderFor(crop?.cropName);
							const badges = buildBadges(crop);
							const availableTxt = `${Number(crop?.quantity || 0)} ${
								crop?.unit || "quintal"
							}`;
							return (
								<div
										...
										{badges.length > 0 && (
											<div className="absolute top-2 left-2 flex flex-wrap gap-2">
												{badges.map((b) => (
													<span
														key={b.text}
														className={`px-2 py-0.5 text-xs font-semibold rounded-full text-white ${b.cls}`}
													>
														{b.text}
													</span>
												))}
